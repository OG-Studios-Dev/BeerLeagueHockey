...
`20260923120000_account_deletion_correction_pass_3.sql`, plus the deployed
...
| **LIVE-13** | Run two overlapping reminder claim workers, then force provider success followed by marker-write failure and retry. | Only one worker claims a row. Retry uses the same Resend idempotency key, query/marker failures return non-200, and a successful marker prevents future claims. |
