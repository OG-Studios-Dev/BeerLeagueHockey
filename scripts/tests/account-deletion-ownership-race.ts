...
async function runRace(label: string, query: string, values: string[]) {
  await sessionA.query('BEGIN');
  await sessionA.query('SELECT public.prepare_account_deletion($1)', [deletingUserId]);
...
try {
  ...
  await runRace(
    'league owner assignment',
    'UPDATE public.leagues SET owner_id = $1 WHERE id = $2',
    [deletingUserId, leagueId],
  );
  await runRace(
    'league creator assignment',
    'UPDATE public.leagues SET created_by = $1 WHERE id = $2',
    [deletingUserId, leagueId],
  );
  await runRace(
    'organization owner assignment',
    'UPDATE public.organizations SET owner_user_id = $1 WHERE id = $2',
  ...
  const organization = await sessionA.query<{ owner_user_id: string }>(
